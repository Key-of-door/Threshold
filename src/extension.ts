...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({ name: 'send_message', label: 'Send message', description: 'Leave a concise request, finding or reply in a Task inbox. Omit taskId for your current Task, or supply a full Task ID from the Project Board to address another Task in this Project. Server binds your real Run as sender. Other Project collaborators can read it; this is not private mail. No approval, Task status change or automatic notification/worker start. If the response is lost, read_messages for your Task or read_project_board(taskId) for the target before retrying; another send creates another row.',
    parameters: Type.Object({ body: Type.String({ minLength: 1, maxLength: 6000 }), taskId: Type.Optional(Type.String({ minLength: 1, maxLength: 36 })) }),
