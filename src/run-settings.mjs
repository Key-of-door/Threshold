...
export function validateModelSettings(model, requested, modelsPath = 'models.json in the service Pi configuration directory') {
  if (!model) throw new Error('Pi did not resolve a model for Run settings');
  const ceilingError = (field, requestedValue, configured) => new Error(`${model.provider}/${model.id}: requested ${field}=${requestedValue}, local Pi catalog/configuration ceiling=${configured ?? 'not declared'}. This is a local validation failure, not a provider rejection. Check ${modelsPath}; correct the capacity declaration only against the provider specification. Existing configuration was not changed.`);
  ...
  if (requested.contextWindow !== undefined && (!Number.isSafeInteger(model.contextWindow) || requested.contextWindow > model.contextWindow))
    throw ceilingError('contextWindow', requested.contextWindow, model.contextWindow);
  if (requested.maxOutputTokens !== undefined) {
    ...
    if (!Number.isSafeInteger(model.maxTokens) || requested.maxOutputTokens > model.maxTokens)
      throw ceilingError('maxTokens', requested.maxOutputTokens, model.maxTokens);
...
export function observedModelSettings(native, requested, modelsPath) {
  if (!native.model) {
    if (Object.keys(requested).length) throw new Error('Pi did not report effective Run settings');
    return null;
  }
  validateModelSettings(native.model, requested, modelsPath);
